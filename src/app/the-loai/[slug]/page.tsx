import { fetchComicsByGenre, fetchGenres } from "@/lib/api";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  try {
    ...
    const genre = genresData.data.items.find((g) => g.slug === slug);
    const name = genre?.name || slug;
    return {
      title: `Thể loại ${name} | MangaHub`,
      description: `Đọc truyện tranh thể loại ${name} online miễn phí tại MangaHub`,
    };
  } catch {
    return { title: "Thể loại | MangaHub" };
  }
}
...
export default async function GenrePage({ params, searchParams }: PageProps) {
  ...
  let data;
  try {
    data = await fetchComicsByGenre(slug, currentPage);
  } catch {
    notFound();
  }
    ...
    <div className="container max-w-screen-xl mx-auto px-4 md:px-8 py-8">
      ...
      <div className="flex flex-col gap-4 mb-8">
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <span>/</span>
          <Link href="/the-loai" className="hover:text-primary transition-colors">
            Thể loại
          </Link>
          <span>/</span>
